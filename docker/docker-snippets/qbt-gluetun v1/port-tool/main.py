...
https://gist.github.com/stumpylog/5dc8f19460459c455d67a03485132187
"""
import enum
import logging
import sys
...
if __name__ == "__main__":
    ...
    _VPN_HOST: Final[str] = os.getenv('GLUETUN_HOST', 'host.docker.internal')
    _VPN_CTRL_PORT: Final[int] = os.getenv('GLUETUN_CTRL_PORT', 8000)
    ...
    try:
        ...
        qbt_client = qbittorrentapi.Client(host=f'http://{_TORRENT_HOST}:{_TORRENT_PORT}',
                                           username = os.getenv('QBT_USERNAME', 'admin'),
                                           password = os.getenv('QBT_PASSWORD', 'adminadmin'),
                                           VERIFY_WEBUI_CERTIFICATE = strtobool(os.getenv('VERIFY_QBT_WEBUI_CERT', 'False')))
